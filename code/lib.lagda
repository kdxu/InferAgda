...
data AList : ℕ → ℕ → Set where
  ...
  _asnoc_/_ : {m : ℕ} {n : ℕ} → (σ : AList m n) → (t' : Type m) →
              (x : Fin (suc m)) → AList (suc m) n -- x を t' にした上で、さらにσを行う代入
\end{code}
}
\agdaIgnore{
\begin{code}
...
flexRigid : {m : ℕ} → (x : Fin m) → (t : Type m) → Maybe (Σ[ n ∈ ℕ ] AList m n)
flexRigid {zero} () t
...
amgu : {m : ℕ} → (s t : Type m) → (acc : Σ[ n ∈ ℕ ] AList m n) → Maybe (Σ[ n ∈ ℕ ] AList m n)
...
\begin{code}
flexRigidLem : {m : ℕ} → {t' : Type m} → ∀{t'' : Type m} → (x : Fin (suc m)) → (t : Type (suc m))
               → check x t ≡ just t' →  TVar ◃ ((t' for x) ◃ (TVar x)) ≡ (λ y → TVar ◃ (t'' for x) y) ◃ t
...
flexRigidLem x (TVar x₁) refl
             | just .(TVar x₂) | [ eq ] | x₂ , refl , proj₃ rewrite proj₃ |  thickxx≡nothing x = refl
...
flexRigidLem {t'' = t''} x (t ⇒ t₁) refl | just (x₁ ⇒ x₂)
             | [ eq ] | .x₁ , .x₂ , refl , b , c rewrite thickxx≡nothing x = cong₂ _⇒_ p q 
...
...                               | nothing = nothing
...                               | just (acc' , ex , eq) with amgu2 s₁ t₁ acc'
...                               | nothing = nothing
...                               | just (acc'' , ex2 , eq2) =
...
...                               | nothing = nothing
amgu2 {suc m} s t (n , σ asnoc r / z) with amgu2 {m} ((r for z) ◃ s) ((r for z) ◃ t) (n , σ)
... | nothing = nothing
... | just ((n' , σ') , ex , eq) =
