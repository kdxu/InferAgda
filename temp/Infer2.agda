...
liftAList1 : {m m' : ℕ} {m'≤′m : m' ≤′ m} → AList m'≤′m  → AList (s≤′s m'≤′m) 
...
substType : {m m' : ℕ} {m'≤′m : m' ≤′ m}→ AList m'≤′m → Type m → Type m' 
substType σ t = sub σ ◃ t

-- substCxt σ Γ : Γ に σ を適用した型環境を返す
substCxt : {m m' n : ℕ} {m'≤′m : m' ≤′ m} → AList m'≤′m → Cxt {m} n → Cxt {m'} n 
...
lookupSubstCxtCommute : {n m' m : ℕ} {m'≤′m : m' ≤′ m} (x : Fin n) (Γ : Cxt {m} n) → (σ : AList m'≤′m) →
...
substWTerm : {m' m n : ℕ} {m'≤′m : m' ≤′ m} → {Γ : Cxt {m} n} → {t : Type m} →
...
unify : {m : ℕ} → Type m → Type m → Maybe (Σ[ n ∈ ℕ ] Σ[ n≤′m ∈ n ≤′ m ] AList n≤′m)
...
inferW : {m n : ℕ} →  (Γ : Cxt {m} n) → (s : WellScopedTerm n) →
         Maybe (Σ[ m' ∈ ℕ ] Σ[ m'≤′s+m ∈ m' ≤′ count s + m ] AList m'≤′s+m × Type m')
inferW {m} Γ (Var x) = just (m , ≤′-refl , anil , lookup x Γ)
inferW {m} Γ (Lam s) with inferW (TVar (fromℕ m) ∷ liftCxt 1 Γ) s -- TVar (fromℕ m) が引数の型
... | nothing = nothing -- s に型がつかなかった
... | just (m' , m'≤′1+s+m , σ , t) rewrite +-suc (count s) m =
  just (m' , m'≤′1+s+m , σ , substType σ tx ⇒ t) -- TVar (fromℕ m) ⇒ t が Lam s の型
...
... | just (m1 , m1≤′s1+m , σ1 , t1) -- s1 の型が t1
      with inferW (substCxt σ1 (liftCxt (count s1) Γ)) s2
... | nothing = nothing -- s2 に型がつかなかった
... | just (m2 , m2≤′s2+m1 , σ2 , t2) -- s2 の型が t2。m2 を App s1 s2 の返り値の型に割り当てる
...
... | just (m3 , m3≤′1+m2 , σ3) rewrite +-comm (count s1) (suc (count s2)) | +-assoc (count s2) (count s1) m =
  just (m3 , ≤′-trans m3≤′1+m2 (≤′-trans (s≤′s m2≤′s2+m1) (s≤′s (k+n≤′k+m (count s2) m1≤′s1+m))) ,
        σ3 ⊹⊹ (σ2' ⊹⊹ σ1') , substType σ3 (TVar (fromℕ m2))) 
  where σ1' : AList (s≤′s (k+n≤′k+m (count s2) m1≤′s1+m))
        σ1' = liftAList (suc (count s2)) σ1
        σ2' : AList (s≤′s m2≤′s2+m1)
        σ2' = liftAList1 σ2
...
test1 = Lam (Var zero)
infer1 : inferW {0} [] test1 ≡ just (1 , ≤′-refl , anil , TVar zero ⇒ TVar zero)
infer1 = refl

test2 : WellScopedTerm 0
test2 = Lam (Lam (Var zero))
infer2 : inferW {0} [] test2 ≡ just (2 , ≤′-refl , anil , TVar zero ⇒ (TVar (suc zero) ⇒ TVar (suc zero)))
infer2 = refl 

test2b : WellScopedTerm 0
test2b = Lam (Lam (Var (suc zero)))
infer2b : inferW {0} [] test2b ≡ just (2 , ≤′-refl , anil , TVar zero ⇒ (TVar (suc zero) ⇒ TVar zero))
infer2b = refl 
...
mutual
  -- σ（または m≤′m'）で再帰。
  liftSub1Commute : ∀ {m' m} {m≤′m' : m ≤′ m'} (σ : AList m≤′m') (x : Fin m') →
                    liftType 1 (sub σ x) ≡ sub (liftAList1 σ) (inject₁ x)
  liftSub1Commute {m'} {.m'} {≤′-refl} anil x = refl
  liftSub1Commute {.(suc n)} {m} {≤′-step m≤′n} (_asnoc_/_ {n} {.m} {.m≤′n} σ t y) x
    with thickInject1 y x
  ... | inj₁ (eq1 , eq2) rewrite eq1 | eq2 = liftSubstCommute (suc zero) σ t
...
--  ... | inj₂ (x' , eq1 , eq2) rewrite eq1 | eq2 = liftSub1Commute σ x'
  ...
  -- c で再帰。σ, x 不変だが lift している。
  liftSubCommute : ∀ {m' m} {m≤′m' : m ≤′ m'} c (σ : AList m≤′m') (x : Fin m') →
...
{-
lem : ∀ {m n} (m≤′n : m ≤′ n) → ≤′-step (s≤′s m≤′n) ≡ s≤′s (≤′-step m≤′n)
lem m≤′n = refl

x1 : {m n : ℕ} (c : ℕ) (m≤′n₁ : m ≤′ n) → c + m ≤′ suc (c + n)
x1 = λ {m n} c (m≤′n : m ≤′ n) → ≤′-step (k+n≤′k+m c m≤′n)

x2 : {m n : ℕ} (c : ℕ) (m≤′n₁ : m ≤′ n) → c + m ≤′ c + suc n
x2 = λ {m n} c (m≤′n : m ≤′ n) → k+n≤′k+m c (≤′-step m≤′n)

x3 : {m n : ℕ} (c : ℕ) (m≤′n₁ : m ≤′ n) → c + m ≤′ suc (c + n)
x3 = λ {m n} c (m≤′n : m ≤′ n) → subst (_≤′_ (c + m)) (+-suc c n) (k+n≤′k+m c (≤′-step m≤′n))
-}

lemma : ∀ {m n} c → (m≤′n : m ≤′ n) → ≤′-step (k+n≤′k+m c m≤′n) ≅ k+n≤′k+m c (≤′-step m≤′n)
lemma zero m≤′n = hrefl
lemma {m} {n} (suc c) m≤′n = hcong' (λ x → c + m ≤′ x) (sym (+-suc c n)) s≤′s (lemma c m≤′n)

liftAListAsnocCommute : ∀ {m n} {m≤′n : m ≤′ n} c (σ : AList m≤′n) (t : Type n) (y : Fin (suc n)) →
                        liftAList c (σ asnoc t / y) ≡ -- ≅
                        hsubst' (_≤′_ (c + m)) (sym (+-suc c n)) AList (lemma c m≤′n)
                                (liftAList c σ asnoc liftType c t / subst Fin (+-suc c n) (inject+' c y))
liftAListAsnocCommute c σ t y = {!!}

liftSubCommute : ∀ {m' m} {m≤′m' : m ≤′ m'} c (σ : AList m≤′m') (x : Fin m') →
                 liftType c (sub σ x) ≡ sub (liftAList c σ) (inject+' c x)
liftSubCommute {m'} {.m'} {≤′-refl} c anil x = {!!}
liftSubCommute {.(suc n)} {m} {≤′-step m≤′n} c (_asnoc_/_ {n} {.m} {.m≤′n} σ t y) x
  with thickInject1 y x
... | inj₁ (eq1 , eq2) rewrite eq1 | eq2 = {!!}
... | inj₂ (x' , eq1 , eq2) rewrite eq1 | liftSubCommute c σ x' | liftAListAsnocCommute c σ t y | eq2 = {!!} -- rewrite eq1 | eq2 = {!!}
  -- rewrite (≅-to-≡ (liftAListAsnocCommute c σ t y)) = ?
--   with thickInject1 y x
-- ... | inj₁ (eq1 , eq2) rewrite eq1 | eq2 = {!!}
-- ... | inj₂ (x' , eq1 , eq2) rewrite eq1 | eq2 = {!liftSubCommute c σ x!}

-- t で再帰。c, σ 不変。
liftSubstCommute : ∀ {m' m} {m≤′m' : m ≤′ m'} c (σ : AList m≤′m') (t : Type m') →
